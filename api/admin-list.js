...
export default async function handler(req,res){
  ...
  if(!process.env.ADMIN_PASSWORD){ // fallback for demo
    if(pw!=='admin123') return res.status(401).json({error:'unauthorized'});
  } else if(pw!==process.env.ADMIN_PASSWORD) return res.status(401).json({error:'unauthorized'});
  const sb=createClient(SUPABASE_URL,SUPABASE_SERVICE_ROLE_KEY);
  const {data}=await sb.from('entries').select('*').eq('donation_confirmed',true).eq('payment_confirmed',false).order('last_bid_at',{ascending:false});
