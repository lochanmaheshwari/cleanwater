import { createClient } from '@supabase/supabase-js';

const SUPABASE_URL = process.env.SUPABASE_URL || 'https://ecvumloyecjefvryjgaf.supabase.co';
const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVjdnVtbG95ZWNqZWZ2cnlqZ2FmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NzY1NDkzOCwiZXhwIjoyMTAzMjMwOTM4fQ.T2Cs93d8QnHcpoMJgtFgUD0chiWSqiPxItzdihmwZko';
const SITE = process.env.SITE || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://cleanwater.lol');

export default async function handler(req,res){
  const id=(req.query.id || req.query.entryId || '').toString();
  if(!id) return res.redirect(302, '/?error=notfound');
  const sb=createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
  let { data: entry } = await sb.from('entries').select('id, status, donation_cents').eq('id', id).single();
  if(!entry) return res.redirect(302, '/?error=notfound');

  // poll briefly if still pending (webhook can lag browser)
  let e=entry;
  for(let i=0;i<10 && e.status==='pending'; i++){
    await new Promise(r=>setTimeout(r,500));
    const { data } = await sb.from('entries').select('id, status, donation_cents').eq('id', id).single();
    if(data) e=data;
  }
  if(e.status==='pending') return res.redirect(302, `/pending.html?id=${encodeURIComponent(e.id)}`);
  if(e.status==='voided') return res.redirect(302, `/?error=voided`);
  // if paid or live, go to donation
  const amount=(e.donation_cents/100).toFixed(2);
  const url = `https://www.every.org/lifestraw-safe-water-fund/f/clean-water-funded-by`
    + `?amount=${encodeURIComponent(amount)}`
    + `&partnerDonationId=${encodeURIComponent(e.id)}`
    + `&webhook_token=${encodeURIComponent(process.env.EVERYORG_WEBHOOK_TOKEN||'')}`
    + `&method=card`
    + `&success_url=${encodeURIComponent(`${SITE}/done.html?id=${encodeURIComponent(e.id)}`)}`;
  return res.redirect(302, url);
}
