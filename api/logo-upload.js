...
// simple in-memory rate limit by IP (per function instance)
const hits = new Map();
...
export default async function handler(req,res){
  ...
  if(recent.length >= 10) return res.status(429).json({error:'rate limited — 10 uploads per minute'});
  recent.push(now);
  hits.set(ip, recent);
  ...
  if(error){
    // fallback: create signed URL for upload via service role
    // older supabase-js: use createSignedUrl for upload? try alternative
