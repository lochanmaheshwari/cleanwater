...
let allEntries = (() => {
  try {
    const cached = JSON.parse(localStorage.getItem('sw_cached_entries_v8') || 'null');
...
let bidsCache = (() => {
  try {
    const cached = JSON.parse(localStorage.getItem('sw_cached_bids_v8') || 'null');
...
async function loadData() {
  ...
  try {
    ...
    if (entries && entries.length > 0) {
      allEntries = entries;
      try {
        localStorage.setItem('sw_cached_entries_v8', JSON.stringify(allEntries));
        localStorage.setItem('sw_cached_bids_v8', JSON.stringify(bidsCache));
      } catch(e) {}
    }
